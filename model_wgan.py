...
class CoGAN(object):
	...
	# input of the generator is the concat of z, y
        self.G1 = self.generator(self.z, self.y, share_params=False, reuse=False, name='G1', is_training=True)
	self.G2 = self.generator(self.z, self.y, share_params=True, reuse=False, name='G2', is_training=True)
	...
	# generate sample
        self.sampler1 = self.generator(self.z, self.y, share_params=True, reuse=True, name='G1', is_training=True)
	self.sampler2 = self.generator(self.z, self.y, share_params=True, reuse=True, name='G2', is_training=True)
	...
	# B1
        self.d1_loss = tf.reduce_mean(self.D1_ - self.D1)
        self.g1_loss = tf.reduce_mean(-self.D1_)
	self.g1_loss_sum = tf.summary.scalar("g1_loss", self.g1_loss)
        self.d1_loss_sum = tf.summary.scalar("d1_loss", self.d1_loss)

        self.d1_loss = self.d1_loss + self.gradient_loss(self.images1, self.G1, name = 'D1')

	# B1
        self.d2_loss = tf.reduce_mean(self.D2_ - self.D2)
        self.g2_loss = tf.reduce_mean(-self.D2_)
	self.g2_loss_sum = tf.summary.scalar("g2_loss", self.g2_loss)
        self.d2_loss_sum = tf.summary.scalar("d2_loss", self.d2_loss)

        self.d2_loss = self.d2_loss + self.gradient_loss(self.images2, self.G2, name = 'D2')
    ...
    def gradient_loss(self, images, Generators, name):
        if mode is 'gp':
            alpha_dist = tf.contrib.distributions.Uniform(low=0., high=1.)
            alpha = alpha_dist.sample((self.batch_size, 1, 1, 1))
            interpolated = images + alpha*(Generators-images)
            _, inte_logit = self.discriminator(interpolated, self.y, share_params=True, reuse=True, name=name)
            gradients = tf.gradients(inte_logit, [interpolated,])[0]
            grad_l2 = tf.sqrt(tf.reduce_sum(tf.square(gradients), axis=[1,2,3]))
            gradient_penalty = tf.reduce_mean((grad_l2-1)**2)
            gp_loss_sum = tf.summary.scalar(name+"gp_loss", gradient_penalty)
            grad = tf.summary.scalar(name+"grad_norm", tf.nn.l2_loss(gradients))

            return lam*gradient_penalty
        else:
            return 0
	...
	d_optim = tf.train.RMSPropOptimizer(config.learning_rate) \
        ...
        if mode is 'regular':
            ...
            with tf.control_dependencies([d_optim]):
                d_optim = tf.tuple(clipped_var_c)

	tf.global_variables_initializer().run()

        self.g1_sum = tf.summary.merge([self.z_sum, self.d1__sum, 
            self.G1_sum, self.g1_loss_sum])
        self.d1_sum = tf.summary.merge([self.z_sum, self.d1_sum])

        self.g2_sum = tf.summary.merge([self.z_sum, self.d2__sum,
            self.G2_sum, self.g2_loss_sum])
        self.d2_sum = tf.summary.merge([self.z_sum, self.d2_sum])
	...
	with tf.variable_scope(name):
		...
		tf.get_variable_scope().reuse_variables()
            ...
            h1 = prelu(conv2d(h0, self.df_dim, name='d'+branch+'_h1_conv', reuse=False),name='d'+branch+'_h1_prelu', reuse=False)
            h1 = tf.reshape(h1, [self.batch_size, -1])            

        # layers that share variables
        h2 = prelu(linear(h1, self.dfc_dim, 'd_h2_lin', reuse=share_params),name='d_h2_prelu', reuse=share_params)

        h3 = linear(h2, 1, 'd_h3_lin', reuse=share_params)
            
        return tf.nn.sigmoid(h3), h3

    def generator(self, z, y=None, share_params=False, reuse=False, name='G',is_training=True):
	...
	# layers that share the variables 
        ...
        h0 = prelu(tf.contrib.layers.batch_norm(linear(z, self.gfc_dim, 'g_h0_lin', reuse=share_params),reuse=share_params, scope='g_bn0',is_training=is_training),name='g_h0_prelu', reuse=share_params)

        h1 = prelu(tf.contrib.layers.batch_norm(linear(z, self.gf_dim*2*s4*s4,'g_h1_lin',reuse=share_params),reuse=share_params,scope='g_bn1',is_training=is_training),
						name='g_h1_prelu', reuse=share_params)
        h1 = tf.reshape(h1, [self.batch_size, s4, s4, self.gf_dim * 2])

        h2 = prelu(tf.contrib.layers.batch_norm(deconv2d(h1, [self.batch_size,s2,s2,self.gf_dim * 2], 
			name='g_h2', reuse=share_params), reuse=share_params,scope='g_bn2',is_training=is_training), name='g_h2_prelu', reuse=share_params)
